...
class LaborFactorTable extends Component {
  ...
  componentDidMount() {
    ...
    const updatedScheduleClassPairs = this.getScheduleClassPairs(
      items,
      updatedSizes
    );
    this.setState(
  ...
  addNewColumn() {
    ...
    const temporarySizes = sizes
      .slice()
      .concat(Math.floor(parseInt(sizes[sizes.length - 1], 10) + 1));
    this.setState(state => ({
  ...
  saveNewColumn() {
    ...
    const newScheduleClassPairs = this.getScheduleClassPairs(
      this.state.data,
      newSizes
    );
    this.setState(() => ({
      ...
      <div>
        {!addingRow ? (
          <div>
            ...
            {editing ? (
              <button onClick={this.setLaborFactors}>
                Update Labor Factors
              </button>
            ) : null}
            {editing ? (
            ...
            {addingColumn ? (
              <button onClick={this.deleteNewColumn}>Delete New Column</button>
            ) : null}
            {addingColumn ? (
              <button onClick={this.saveNewColumn}>Save New Column</button>
            ) : null}
            {!addingRow ? (
                ...
                <tr>
                  ...
                  {addingColumn ? (
                    <th>
                      ...
                      <input
                        type="text"
                        defaultValue={tempSizes[tempSizes.length - 1]}
                        onChange={event => {
                ...
                {scheduleClassPairs.map((pair, outerIndex) => (
                  <tr onClick={this.editLaborFactor} key={outerIndex}>
                    ...
                    {sizes.map((size, innerIndex) => (
                      <td key={innerIndex}>
                        {!editing ? (
                          <span onClick={this.editLaborFactors}>
                            {scheduleClassPairs[outerIndex][size]}
                          </span>
                        ) : (
                          <input
                            type="text"
                            defaultValue={scheduleClassPairs[outerIndex][size]}
                            onChange={event => {
                              this.changeLaborFactor(
                                event.target.value,
                                outerIndex,
                                size
                              );
